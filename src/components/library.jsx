...
const Library = () => {
  ...
  return (
    <div className='grid drag py-10 grid-cols-2 sm:grid-cols-3 gap-y-5 gap-x-2 sm:gap-x-5 lg:grid-cols-4 xl:grid-cols-5' ref={gridRef}>
      {data.map((image) => (
        <div key={image.id} data-id={image.id} className="grid-square">
          <img className='rounded-2xl w-full bg-cover bg-center bg-no-repeat h-[20em]' src={image.img} alt="" />
        </div>
      ))}
    </div>
  );
